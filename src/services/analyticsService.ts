...
export default class AnalyticsService {
  ...
  async getStudentAnalytics(studentId: string) {
    try {
      ...
      // Get recent activity
      const recentActivity = await Progress.find({ userId: studentId })
        .sort({ createdAt: -1 })
        .limit(5)
  ...
  private async getCategoryPerformance(userId: string) {
    try {
  ...
  private async getTimeData(userId: string) {
    try {
      // Get date 4 weeks ago
      const fourWeeksAgo = new Date();
      fourWeeksAgo.setDate(fourWeeksAgo.getDate() - 28);
      
      const weeklyData = await Attempt.aggregate([
        { 
          $match: { 
      ...
      logger.error('Error getting time data:', error);
      return this.generateMockTimeData();
    }
  }
  ...
  private generateMockCategoryData() {
    return [
      { 
        category: 'Math', 
        totalAttempts: 20, 
  ...
  async getTeacherAnalytics(teacherId: string) {
    try {
      ...
      const classroomPerformance = await Promise.all(classrooms.map(async classroom => {
        const studentIds = classroom.students && Array.isArray(classroom.students) 
          ? classroom.students 
          : [];
          
        // Get attempts for students in this classroom
        const classroomAttempts = await Attempt.find({
          user: { $in: studentIds }
        }).lean();
      ...
      logger.error('Error getting teacher analytics:', error);
  ...
  async getStudentAnalyticsUsingProgress(userId: string) {
    try {
